...
const char * const BITCOIN_CONF_FILENAME = "alp-litecoin.conf";
...
#else
    const char* pszModule = "alp-litecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "alp-alp-litecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/alp-alp-litecoin";
#else
    // Unix-like
    return pathRet / ".alp-litecoin";
#endif
#endif
}
