...
if command -v apt &>/dev/null; then
  apt update
  apt install -y build-essential libtool autotools-dev automake pkg-config bsdmainutils curl git autoconf automake libtool pkg-config m4 gettext autopoint \
